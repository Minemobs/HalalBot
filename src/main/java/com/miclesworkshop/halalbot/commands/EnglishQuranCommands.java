...
public class EnglishQuranCommands extends AbstractCommands {
    private String[][][] data; // first dimension: surah index. second dimension: ayah index. third dimension: arabic/english

    public EnglishQuranCommands(HalalBot bot) {
        super(bot);
        try (ObjectInputStream ois = new ObjectInputStream(EnglishQuranCommands.class.getResourceAsStream("/halalbot_quran_data.bin"))) {
